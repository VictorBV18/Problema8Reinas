...
public class Problema8Reinas {
    ...
    static void visualizacion(int[] tablero) {
        ...
        for (int i = 0; i < 8; i++) { //i representa las celdas horizontales (FILAS)
            for (int j = 0; j < 8; j++) {//j representa las celdas verticales   (COLUMNAS)
                if (tablero[i] == j) {
    ...
    static void soluciones() {
        ...
        for (int a = 0; a < 8; a++)
            for (int b = 0; b < 8; b++)
                for (int c = 0; c < 8; c++)
                    for (int d = 0; d < 8; d++)//AQUI ESTOY FORZANDO CON 8 LETRAS PARA QUE VAYA RECORRIENDO UNA CASILLA POR CASILLA :)
                        for (int e = 0; e < 8; e++)
                            for (int f = 0; f < 8; f++)
                                for (int g = 0; g < 8; g++)
                                    for (int h = 0; h < 8; h++){
    ...
    public static void main(String[] args) {
        ...
        visualizacion(new int[]{2, 5, 7, 1, 3, 0, 6, 4});
